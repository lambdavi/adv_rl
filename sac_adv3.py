...
class Args:
    ...
    # Dynamic clustering arguments
    use_dynamic_clustering: bool = False
    """if toggled, use adaptive clustering instead of fixed clusters"""
    min_clusters: int = 3
    """minimum number of clusters to maintain"""
    max_clusters: int = 50
    """maximum number of clusters to maintain"""
    cluster_quality_threshold: float = 0.3
    """minimum silhouette score to maintain cluster quality"""
    adaptive_threshold_percentile: float = 75.0
    """percentile for adaptive distance threshold calculation"""
    cluster_evaluation_frequency: int = 1000
    """how often to evaluate and adjust clusters"""



class DynamicFailureClusterBuffer:
    def __init__(self, max_clusters=20, distance_threshold=0.5, 
                 use_dynamic=False, min_clusters=3, max_clusters_dynamic=50,
                 quality_threshold=0.3, adaptive_percentile=75.0):
        self.clusters = []  # List of (centroid, states, weight, cluster_id)
        self.max_clusters = max_clusters
        self.distance_threshold = distance_threshold
        self.use_dynamic = use_dynamic
        self.min_clusters = min_clusters
        self.max_clusters_dynamic = max_clusters_dynamic
    ...
    def _calculate_adaptive_threshold(self):
        """Calculate adaptive distance threshold based on current data distribution"""
        ...
        for i in range(len(self.clusters)):
            for j in range(i + 1, len(self.clusters)):
                dist = np.linalg.norm(self.clusters[i][0] - self.clusters[j][0])
                distances.append(dist)
        
        if not distances:
            return self.distance_threshold
            
        # Use percentile of distances as adaptive threshold
        adaptive_threshold = np.percentile(distances, self.adaptive_percentile)
        return max(adaptive_threshold * 0.5, self.distance_threshold * 0.1)  # Ensure reasonable bounds
    ...
    def _calculate_silhouette_score(self):
        ...
        try:
            ...
            for cluster_idx, (centroid, states, weight, _) in enumerate(self.clusters):
                if len(states) > 0:
                    all_states.extend(states)
                    labels.extend([cluster_idx] * len(states))
            
            if len(all_states) < 2 or len(set(labels)) < 2:
                return 1.0
            ...
            score = silhouette_score(all_states, labels)
            return max(0.0, score)  # Ensure non-negative
            
        except ImportError:
            # Fallback: simple quality metric based on cluster separation
            if len(self.clusters) < 2:
                return 1.0
            ...
            avg_separation = total_dist / count
            # Normalize by state dimension (rough approximation)
            state_dim = len(self.clusters[0][0]) if self.clusters else 1
            normalized_separation = avg_separation / (state_dim ** 0.5)
            return min(1.0, normalized_separation)
    
    def _evaluate_and_adjust_clusters(self, current_step):
        """Evaluate cluster quality and adjust if needed"""
        if not self.use_dynamic or current_step - self.last_evaluation_step < 1000:
            return
        ...
        # Adjust distance threshold adaptively
        if self.use_dynamic:
            self.distance_threshold = self._calculate_adaptive_threshold()
        
        # If quality is poor and we have too many clusters, merge some
        if quality < self.quality_threshold and len(self.clusters) > self.min_clusters:
            self._merge_poor_quality_clusters()
        
        # If quality is good and we have room, consider splitting large clusters
        elif quality > self.quality_threshold * 1.5 and len(self.clusters) < self.max_clusters_dynamic:
            self._split_large_clusters()
    
    def _merge_poor_quality_clusters(self):
        ...
        if min_distance < self.distance_threshold * 2:
            centroid1, states1, weight1, _ = self.clusters[merge_i]
            centroid2, states2, weight2, _ = self.clusters[merge_j]
            ...
            self.clusters.append((new_centroid, all_states, new_weight, self.cluster_id_counter))
            self.cluster_id_counter += 1
    
    def _split_large_clusters(self):
        """Split clusters that are too large or spread out"""
        for i, (centroid, states, weight, cluster_id) in enumerate(self.clusters):
            if len(states) > 20:  # Only split large clusters
                states_array = np.array(states)
                
                # Check if cluster is spread out (high variance)
                variance = np.var(states_array, axis=0).mean()
                if variance > self.distance_threshold ** 2:
                    ...
                    try:
                        ...
                        cluster1_states = [states[j] for j in range(len(states)) if labels[j] == 0]
                        cluster2_states = [states[j] for j in range(len(states)) if labels[j] == 1]
                        
                        if len(cluster1_states) > 0 and len(cluster2_states) > 0:
                            centroid1 = np.mean(cluster1_states, axis=0)
                            centroid2 = np.mean(cluster2_states, axis=0)
                            weight1 = len(cluster1_states)
                            weight2 = len(cluster2_states)
                            
                            # Replace original cluster with two new ones
                            self.clusters[i] = (centroid1, cluster1_states, weight1, self.cluster_id_counter)
                            self.clusters.append((centroid2, cluster2_states, weight2, self.cluster_id_counter + 1))
                            self.cluster_id_counter += 2
                            
                            # Check if we've reached max clusters
                            if len(self.clusters) >= self.max_clusters_dynamic:
                                break
                                
                    except ImportError:
                        # Fallback: simple split based on distance from centroid
                        distances = [np.linalg.norm(np.array(state) - centroid) for state in states]
                        median_dist = np.median(distances)
                        
                        cluster1_states = [states[j] for j in range(len(states)) if distances[j] <= median_dist]
                        cluster2_states = [states[j] for j in range(len(states)) if distances[j] > median_dist]
    ...
    def add_failure_state(self, state, current_step=0):
        state = np.array(state)
        
        # Evaluate and adjust clusters periodically
        self._evaluate_and_adjust_clusters(current_step)
        
        # Find closest cluster
        min_distance = float('inf')
        closest_cluster_idx = -1
        
        for i, (centroid, states, weight, _) in enumerate(self.clusters):
        ...
        if closest_cluster_idx >= 0 and min_distance < self.distance_threshold:
            centroid, states, weight, cluster_id = self.clusters[closest_cluster_idx]
            states.append(state)
            # Update centroid
            new_centroid = np.mean(states, axis=0)
            self.clusters[closest_cluster_idx] = (new_centroid, states, weight + 1, cluster_id)
        else:
            # Create new cluster
            if len(self.clusters) >= (self.max_clusters_dynamic if self.use_dynamic else self.max_clusters):
                # Merge closest clusters
                self._merge_closest_clusters()
            self.clusters.append((state, [state], 1, self.cluster_id_counter))
            self.cluster_id_counter += 1
    ...
    def compute_penalty(self, states):
        ...
        for centroid, states_list, weight, _ in self.clusters:
    ...
    def sample_danger_states(self, n_samples):
        ...
        weights = [weight for _, _, weight, _ in self.clusters]
        ...
        for _ in range(n_samples):
            ...
            centroid, states, weight, _ = self.clusters[cluster_idx]
    ...
    def get_cluster_stats(self):
        ...
        total_states = sum(len(states) for _, states, _, _ in self.clusters)
        avg_cluster_size = total_states / len(self.clusters)
        quality_score = self._calculate_silhouette_score()
        adaptive_threshold = self._calculate_adaptive_threshold() if self.use_dynamic else self.distance_threshold
...
class FailureClusterBuffer(DynamicFailureClusterBuffer):
    def __init__(self, max_clusters=20, distance_threshold=0.5):
        super().__init__(max_clusters, distance_threshold, use_dynamic=False)
...
if __name__ == "__main__":
    ...
    if args.use_failure_buffer:
        ...
        if args.use_dynamic_clustering:
            print(f"    🔄 DYNAMIC CLUSTERING: ENABLED")
            print(f"      Min clusters: {args.min_clusters}")
            print(f"      Max clusters: {args.max_clusters}")
            print(f"      Quality threshold: {args.cluster_quality_threshold}")
            print(f"      Adaptive percentile: {args.adaptive_threshold_percentile}")
            print(f"      Evaluation frequency: {args.cluster_evaluation_frequency}")
        else:
            print(f"    📊 STATIC CLUSTERING: FIXED {args.max_failure_clusters} CLUSTERS")
    else:
    ...
    if args.use_failure_buffer:
        failure_buffer = DynamicFailureClusterBuffer(
            max_clusters=args.max_failure_clusters,
            distance_threshold=args.failure_distance_threshold,
            use_dynamic=args.use_dynamic_clustering,
            min_clusters=args.min_clusters,
            max_clusters_dynamic=args.max_clusters,
            quality_threshold=args.cluster_quality_threshold,
            adaptive_percentile=args.adaptive_threshold_percentile
        )
    else:
        failure_buffer = None
            ...
            for env_idx in range(args.num_envs):
                ...
                if is_failure and len(trajectory_buffer[env_idx]) > 0:
                    ...
                    for state in final_states:
                        failure_buffer.add_failure_state(state, global_step)
        ...
        if global_step > args.learning_starts:
            ...
            if args.use_failure_buffer and failure_buffer is not None and failure_buffer.clusters:
                ...
                if global_step % 100 == 0:
                    ...
                    # Log dynamic clustering statistics if enabled
                    if args.use_dynamic_clustering:
                        cluster_stats = failure_buffer.get_cluster_stats()
                        writer.add_scalar("failure_buffer/cluster_quality", cluster_stats['quality_score'], global_step)
                        writer.add_scalar("failure_buffer/adaptive_threshold", cluster_stats['adaptive_threshold'], global_step)
                        writer.add_scalar("failure_buffer/avg_cluster_size", cluster_stats['avg_cluster_size'], global_step)
                        writer.add_scalar("failure_buffer/total_states", cluster_stats['total_states'], global_step)
            else:
